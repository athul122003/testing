...
import { ArrowLeft, Save, Eye, Bold, Italic, List, Link, ImageIcon, Upload } from "lucide-react"
...
export function BlogForm({ setActivePage, editingBlog, setEditingBlog }: BlogFormProps) {
  ...
  const insertImage = () => {
    ...
    if (textarea) {
      ...
      const newContent = formData.content.substring(0, start) + imageText + formData.content.substring(start)
      setFormData({ ...formData, content: newContent })
  ...
  const renderPreview = () => {
    return (
      <div className="prose prose-gray dark:prose-invert max-w-none">
        <h1 className="text-3xl font-bold mb-4">{formData.title || "Blog Title"}</h1>
        {formData.excerpt && <p className="text-lg text-gray-600 dark:text-gray-400 italic mb-6">{formData.excerpt}</p>}
        <div className="space-y-4">
          {formData.content.split("\n").map((line, index) => {
            ...
            if (line.startsWith("![")) {
              ...
              if (match) {
                return (
                  <div key={`image-${key}`} className="my-4 p-4 bg-blue-100 dark:bg-blue-900/30 rounded-lg text-center">
                    <ImageIcon className="h-8 w-8 mx-auto mb-2 text-blue-500" />
                    <p className="text-sm text-blue-700 dark:text-blue-300">📷 {match[1] || "Image"}</p>
                    <p className="text-xs text-blue-600 dark:text-blue-400">{match[2]}</p>
                  </div>
                )
              }
            }
            ...
            if (line.startsWith("[") && line.includes("](")) {
              ...
              if (match) {
                return (
                  <p key={`link-${key}`}>
                    <a href={match[2]} className="text-blue-500 hover:underline">
                      {match[1]}
                    </a>
                  </p>
                )
            ...
            return line ? <p key={`plain-${key}`}>{line}</p> : <br key={`br-${key}`} />
          })}
        </div>
        {formData.tags && (
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            {editingBlog ? "Update your blog post" : "Write and publish your blog content"}
          </p>
        </div>
      </div>
      ...
      <Tabs defaultValue="edit" className="w-full">
        ...
        <TabsContent value="edit" className="space-y-6">
          <Card className="border-0 shadow-lg bg-white dark:bg-gray-900">
            <CardHeader>
              <CardTitle className="text-xl text-gray-900 dark:text-white">Blog Information</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center hover:border-gray-400 dark:hover:border-gray-500 transition-colors cursor-pointer">
                  ...
                  <p className="text-gray-600 dark:text-gray-400">Upload featured image</p>
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-xl overflow-hidden">
                  <div className="flex items-center gap-2 p-3 border-b bg-gray-50 dark:bg-gray-800">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => formatText("list")} title="List">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  Tip: Select text and use formatting buttons, or use markdown syntax directly
                </div>
              </div>
