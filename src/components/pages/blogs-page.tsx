...
import { Plus, Eye, FileText, Calendar, User } from "lucide-react"
...
const initialBlogs = [
  {
    ...
    excerpt: "Learn the fundamentals of React and build modern web applications with this comprehensive guide.",
    date: "2024-01-15",
    author: "Admin",
    status: "Published",
    views: 1250,
  ...
  },
  {
    ...
    excerpt: "Discover the latest CSS features and techniques for creating beautiful, responsive web designs.",
    date: "2024-01-10",
    author: "Admin",
    status: "Draft",
    views: 0,
...
export function BlogsPage({ setActivePage, setEditingBlog }: BlogsPageProps) {
  ...
  const handlePublishBlog = (blogId) => {
    setBlogs(blogs.map((blog) => (blog.id === blogId ? { ...blog, status: "Published" } : blog)))
    setIsDetailOpen(false)
  }
          ...
          >
            ...
            <CardContent className="p-6">
              <div className="space-y-4">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2 line-clamp-2">{blog.title}</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-3">{blog.excerpt}</p>
                </div>
      ...
      {blogs.length === 0 && (
        <Card className="border-0 shadow-lg bg-white dark:bg-gray-900">
          <CardContent className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-500">Create your first blog post to get started!</p>
          </CardContent>
        </Card>
      )}
          ...
          {selectedBlog && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-3 gap-6">
                <div className="col-span-2 space-y-4">
                  <div>
                    ...
                    <div className="prose prose-gray dark:prose-invert max-w-none">
                      <p className="text-gray-600 dark:text-gray-400">{selectedBlog.content}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Post Details</h3>
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        <User className="h-4 w-4 text-gray-500" />
                        <span className="text-sm">Author: {selectedBlog.author}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-gray-500" />
                        <span className="text-sm">Published: {selectedBlog.date}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Eye className="h-4 w-4 text-gray-500" />
                        <span className="text-sm">Views: {selectedBlog.views}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <Badge className={getStatusColor(selectedBlog.status)} variant="outline">
                      {selectedBlog.status}
                    </Badge>
                  </div>
                </div>
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t">
                ...
                <Button variant="outline" onClick={() => handleEditBlog(selectedBlog)}>
                  Edit Post
                </Button>
                <Button variant="destructive" onClick={() => handleDeleteBlog(selectedBlog.id)}>
                  Delete Post
                </Button>
              </div>
            </div>
