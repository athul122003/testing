...
export function GlobalSearch({ setActivePage }: GlobalSearchProps) {
  ...
  const filteredItems = searchItems.filter((item) => item.title.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  return (
    <>
      <div className="relative max-w-md">
        ...
        <Input placeholder="Search everything..." className="pl-10" onClick={() => setOpen(true)} />
      </div>
